...
public class MyPageController {
    ...
    @GetMapping(value = "/address/account")
    @Operation(summary = "mypage 배송지, 환불 계좌 조회 page")
    public ApiResponse<MemberResponseDTO.AddressAndAccountDTO> getAddressAndAccount(Authentication authentication){
        Member member = memberQueryService.findMemberById(Long.valueOf(authentication.getName().toString())).orElseThrow(() -> new MemberHandler(ErrorStatus.MEMBER_NOT_FOUND));
        List<Account> accountList = memberQueryService.findAccountById(member.getId());
        List<Address> addressList = memberQueryService.findAllAddressById(member.getId());
